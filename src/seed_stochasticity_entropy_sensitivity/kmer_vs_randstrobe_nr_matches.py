...
def compute_frac_matches_per_mut_rate(args, N_sim, outfile):
    ...
    for m in N_m:
        ...
        for n in range(N_sim):
            ...
            seeds1 = [s for i, s in kmers_pos1.items() if i <= w]  # i+k < 2*w] 
            seeds2 = [s for i, s in kmers_pos2.items() if i <= w] # i+k < 2*w] 
    ...
    for m in N_m:
        ...
        for n in range(N_sim):
            # print('for n=', n)
            seq1 ,seq2 = seq_pairs[m][n]
            hybridstrobes_pos1 = indexing.minstrobes(seq1, k, w_min, w_max, 1)
            # print(hybridstrobes_pos1)
            hybridstrobes_pos2 = indexing.minstrobes(seq2, k, w_min, w_max, 1)
            
            # check the seeds within the window 
            seeds1 = [s for i_tuple, s in hybridstrobes_pos1.items() if i_tuple[0] <= w] # i_tuple[2] + k//2 < 2*w] 
            seeds2 = [s for i_tuple, s in hybridstrobes_pos2.items() if i_tuple[0] <= w] # i_tuple[2] + k//2 < 2*w] 
            # print(len(seeds1))
            matches = set(seeds1) & set(seeds2)
            n_hits = len(matches)
            if n_hits > 0:
    ...
    for m in N_m:
        ...
        for n in range(N_sim):
            ...
            # check the seeds within the window 
            seeds1 = [s for i_tuple, s in altstrobes_pos1.items() if i_tuple[0] <= w] # i_tuple[2] + k//2 < 2*w] 
            seeds2 = [s for i_tuple, s in altstrobes_pos2.items() if i_tuple[0] <= w] # i_tuple[2] + k//2 < 2*w] 
            # print(len(seeds1))
            matches = set(seeds1) & set(seeds2)
            n_hits = len(matches)
            if n_hits > 0:
...
def plot_p_match(args, input_csv, plot_file_prefix , x="k_2/k"):
    ...
    g = sns.relplot(data=indata, x="m", y="N_m", hue="seeding", kind='line', facet_kws=dict(sharex=False), hue_order = ['k-mers', 'minstrobes', 'hybridstrobes', 'randstrobes', 'mixedstrobes', 'altstrobes']) #s=80, 
    # g.set(yscale="log")
...
#    plot_p_match(args, args.outcsv, args.outplot_prefix)
