...
def seq_to_altstrobes_continous_iter(seq: str, k_size: int, strobe_w_min_offset: int,
                                     strobe_w_max_offset: int, prime: int, w: int,
                                     order: int, k_boundary: int,
                                     arg: int=0) -> Iterator[tuple]:
    """
    Iterator for creation of altstrobes of any order
    ...
    assert order == 2, "Continous altstrobes are just implemented as order 2"
    ...
    for p1, (p1, hash_m1) in enumerate(hash_seq_lists_thinned[0]):  # [:-k_size]:
        ...
        if hash_m1 // 100 % 2 == arg: # first k_size1, then k_size2 (e.g. 10-20)
            ...
            for index_order in range(1, order):
                min_index, min_value = argmin([
                    (min_hash_val + hash_seq_lists[k_size_2_selection][i][1]) % prime
                    for i in range(*windows[index_order-1])
                ])

                min_hash_val = min_hash_val + (index_order * (-1)**index_order) * hash_seq_lists[k_size_2_selection][windows[index_order-1][0] + min_index][1]
            ...
            min_values = []
            min_hash_val = hash_seq_lists[k_size_2_selection][p1][1]
            for index_order in range(1, order):
                min_index, min_value = argmin([
                    (min_hash_val + hash_seq_lists[k_size_selection][i][1]) % prime
                    for i in range(*windows[index_order-1])
                ])

                min_hash_val = min_hash_val + (index_order * (-1)**index_order) * hash_seq_lists[k_size_selection][windows[index_order-1][0] + min_index][1]
...
def seq_to_altstrobes_continous_iter_strings(seq: str, k_size: int, strobe_w_min_offset: int,
                                     strobe_w_max_offset: int, prime: int, w: int,
                                     order: int, k_boundary: int,
                                     arg: int=0) -> Iterator[tuple]:
    """
    ...
    hash_seq_lists_thinned = []
    for k1 in range(k_boundary, k_size-k_boundary+1):
        hash_seq_lists.append([(i, seq[i:i+k1]) for i in range(len(seq) - k1 + 1)])
        strobe_w_min_offsets.append(int(strobe_w_min_offset - k1 + k_size/2))
        strobe_w_max_offsets.append(int(strobe_w_max_offset - k1 + k_size/2))
    ...
    for p1, (p1, m1) in enumerate(hash_seq_lists_thinned[0]):  # [:-k_size]:
        ...
        if (k_boundary < 3) or (hash(m1) // 100 % 2 == arg): # first k_size1, then k_size2 (e.g. 10-20)
            ...
            for index_order in range(1, order):
                min_index, min_value = argmin([
                    hash(strobemer + hash_seq_lists[k_size_2_selection][i][1])
                    for i in range(*windows[index_order-1])
                ])

                strobemer = strobemer + hash_seq_lists[k_size_2_selection][windows[index_order-1][0] + min_index][1]
                index.append(range(min_index+windows[index_order-1][0], min_index+windows[index_order-1][0]+k_size2))
            ...
            index = [range(p1, p1+k_size2)]
            strobemer = hash_seq_lists[k_size_2_selection][p1][1]
            for index_order in range(1, order):
                min_index, min_value = argmin([
                    hash(strobemer + hash_seq_lists[k_size_selection][i][1])
                    for i in range(*windows[index_order-1])
                ])

                strobemer = strobemer + hash_seq_lists[k_size_selection][windows[index_order-1][0] + min_index][1]
                index.append(range(min_index+windows[index_order-1][0], min_index+windows[index_order-1][0]+k_size1)) # -

        #print(index, start, end, strobemer)
        yield index, strobemer


def altstrobes_continous(seq: str, k_size: int, strobe_w_min_offset: int,
                         strobe_w_max_offset: int, w: int, order: int = 2,
    ...
    altstrobes = {tuple(index): h for index, h in seq_to_altstrobes_continous_iter_strings(
        seq, k_size, strobe_w_min_offset, strobe_w_max_offset, prime, w, order, k_boundary, arg
    )}
    return altstrobes


def altstrobes_continous_iter(seq: str, k_size: int, strobe_w_min_offset: int,
                              strobe_w_max_offset: int, w: int, order: int = 2,
    ...
    for i in range(0, len(seq), buffer_size):
        substring = seq[i:i+buffer_size]
        for p, m in altstrobes_continous(
                substring, k_size, strobe_w_min_offset, strobe_w_max_offset,
...
def seq_to_mixedstrobes_iter(method1: str, method2: str, seq: str,
    ...
    if "altstrobes" in (method1, method2):
        ...
        required_length1 = order * strobe_w_max_offset - strobe_w_max_offset1
        required_length2 = order * strobe_w_max_offset - strobe_w_max_offset2
    if "altstrobes_continous" in (method1, method2):
        k_boundary = 5
        hash_seq_lists = []
        strobe_w_min_offsets = []
        strobe_w_max_offsets = []
    ...
    for (p1, hash_m1) in hash_seq_list_thinned:  # [:-k_size]:
        ...
        if hash_m1//100 % denominator < numerator:  # pick method1
            ...
            elif method1 == "altstrobes_continous":
                if p1 >= min(len(hash_seq_lists[0]) - (order-1)*k_boundary, len(hash_seq_lists[-1]) - (order-1)*(order*k_size-k_boundary)):
                    break
                index, min_hash_val = sample_altstrobes_continous(seq, hash_seq_lists, p1, hash_m1, order*k_size, k_size_options, strobe_w_min_offsets, strobe_w_max_offsets, prime, order, k_boundary)
            else:
            ...
            if method2 == "randstrobes":
                ...
                index, min_hash_val = sample_altstrobes(seq, hash_seq_list1, hash_seq_list2, p1, hash_m1, k_size1, k_size2, strobe_w_min_offset1, strobe_w_max_offset1,
                                                        strobe_w_min_offset2, strobe_w_max_offset2, strobe_w_max_offset, prime, order, required_length1, required_length2)
            elif method2 == "altstrobes_continous":
                if p1 >= min(len(hash_seq_lists[0]) - (order-1)*k_boundary, len(hash_seq_lists[-1]) - (order-1)*(order*k_size-k_boundary)):
                    break
                index, min_hash_val = index, min_hash_val = sample_altstrobes_continous(seq, hash_seq_lists, p1, hash_m1, order*k_size, k_size_options, strobe_w_min_offsets, strobe_w_max_offsets, prime, order, k_boundary)
            else:
                print("Seeding Technique not known")
                raise NotImplementedError
...
def sample_altstrobes_continous(seq: str, hash_seq_lists: list, p1: int,
    ...
    """
    """
    hash_m1 = hash_seq_lists[0][p1][1]  # not necessary, but guarantees identical values to altstrobes_continous
    k_size_selection = hash_m1 % k_size_options
    k_size_2_selection = -(k_size_selection+1)
    k_size1 = k_size_selection + k_boundary
    k_size2 = k_size - k_size1
