...
static inline bool equal_ends(const nam &a, const nam &b)
{
    return ( (a.query_e == b.query_e) && (a.ref_e == b.ref_e) );
}

static inline bool compareByReferenceEnd (const nam &a, const nam &b)
{
  return (a.ref_e < b.ref_e);
}

static inline bool compareByQueryEnd (const nam &a, const nam &b)
{
  return (a.query_e < b.query_e);
}

static inline std::vector<nam> remove_duplicates(std::vector<nam> &nams){
    std::stable_sort(nams.begin(), nams.end(), compareByQueryEnd);
    std::stable_sort(nams.begin(), nams.end(), compareByReferenceEnd);
    // std::cout << nams.size() << std::endl;
    auto last = std::unique(nams.begin(), nams.end(), equal_ends);
    nams.erase(last, nams.end());
    // std::cout << nams.size() << std::endl;
...
void print_usage() {
    ...
    std::cerr << "\t-i Perform only indexing (seeding) without computing NAMs or aligning sequences (for benchmarking only)\n";
...
int main (int argc, char *argv[])
{
    ...
    int w_min = k+1;
    int w_max = 70;
    int strobe_fraction = 60;
    int n_threads = 3;
    bool only_seeding = false;
    bool unique = false;
    bool output_specified = false;
    bool ultra_output = false;
    bool wmin_set = false;
    bool strobemer_fraction_set = false;
    ...
    while (opn < argc) {
        ...
        if (argv[opn][0] == '-') {
            if (argv[opn][1] == 'n') {
                ...
                strobe_fraction = std::stoi(argv[opn + 1]);
                opn += 2;
                flag = true;
                strobemer_fraction_set = true;
            } else if (argv[opn][1] == 'c') {
                choice = argv[opn + 1];
                opn += 2;
                flag = true;
            } else if (argv[opn][1] == 'i') {
                only_seeding = true;
                opn += 1;
                flag = true;
    ...
    if (!strobemer_fraction_set && (choice == "mixedrandstrobes" || choice == "mixedhybridstrobes" || choice == "mixedminstrobes")){
        std::cout << "Strobemer fraction not specified. Continuing with default value of 60 (60%)!" << std::endl;
    }

    if (choice == "altstrobes"){
        if (k2 == 0){
            k2 = 2*k;
            std::cout << "Length of second altstrobe seed not specified. Continuing with default value of 2*k!" << std::endl;
        }
        if (n % 2 == 1){
            n = n - 1;
    ...
    if (choice == "mixedrandstrobes" || choice == "mixedhybridstrobes" || choice == "mixedminstrobes"){
        std::cout << "strobe_fraction: " << strobe_fraction << std::endl;
    }
...
//    assert(k <= (w/2)*w_min && "k should be smaller than (w/2)*w_min to avoid creating short strobemers");
    ...
    assert(strobe_fraction >= 0 && strobe_fraction <= 100 && "Strobe fraction needs to be between 0 (0%) and 100 (100%)!");
    int filter_nams = 0;
...
//   std::string choice = "randstrobes";
    ...
    if (choice == "kmers" ){
        ...
        if (n == 2 ){
            ...
            for (auto x : ref_seqs){
                mers_vector hybridstrobes; // pos, chr_id, kmer hash value
                hybridstrobes = seq_to_hybridstrobes(n, k, w_min, w_max, x.second, x.first);
                tmp_index[x.first] = hybridstrobes;
            }
        }
    }
    else if (choice == "mixedhybridstrobes" ){
        if (n == 2 ){
            for (auto x : ref_seqs){
                mers_vector mixedhybridstrobes2; // pos, chr_id, kmer hash value
                mixedhybridstrobes2 = seq_to_mixedhybridstrobes2(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedhybridstrobes2;
            }
        }
        else if (n == 3){
            for (auto x : ref_seqs){
                mers_vector mixedhybridstrobes3; // pos, chr_id, kmer hash value
                mixedhybridstrobes3 = seq_to_mixedhybridstrobes3(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedhybridstrobes3;
            }
        }
        else {
            for (auto x : ref_seqs){
                mers_vector mixedhybridstrobes; // pos, chr_id, kmer hash value
                mixedhybridstrobes = seq_to_mixedhybridstrobes(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedhybridstrobes;
            }
        }
    }
...
//        else if (n == 3){ for (auto x : ref_seqs){generate_minstrobe3_index(h, k, x.second, x.first);}}
    ...
    else if (choice == "mixedminstrobes" && n == 2){
        for (auto x : ref_seqs){
            mers_vector mixedminstrobes; // pos, chr_id, kmer hash value
            mixedminstrobes = seq_to_mixedminstrobes2(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
            tmp_index[x.first] = mixedminstrobes;
        }
    }
...
//                std::cout << x.first << " : " << acc_map[x.first] << std::endl;
            ...
            for (auto x : ref_seqs){
                mers_vector randstrobes; // pos, chr_id, kmer hash value
//                #pragma omp parallel for
//                std::cout << x.first << " : " << acc_map[x.first] << std::endl;
                randstrobes = seq_to_randstrobes(n, k, w_min, w_max, x.second, x.first);
                tmp_index[x.first] = randstrobes;
            }
        }
    }
    else if (choice == "mixedrandstrobes" ){
        if (n == 2 ){
            for (auto x : ref_seqs){
                mers_vector mixedrandstrobes2; // pos, chr_id, kmer hash value
                mixedrandstrobes2 = seq_to_mixedrandstrobes2(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedrandstrobes2;
            }
        }
        else if (n == 3 ){
            for (auto x : ref_seqs){
                mers_vector mixedrandstrobes3; // pos, chr_id, kmer hash value
                mixedrandstrobes3 = seq_to_mixedrandstrobes3(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedrandstrobes3;
            }
        }
        else {
            for (auto x : ref_seqs){
                mers_vector mixedrandstrobes; // pos, chr_id, kmer hash value
                mixedrandstrobes = seq_to_mixedrandstrobes(n, k, w_min, w_max, strobe_fraction, x.second, x.first);
                tmp_index[x.first] = mixedrandstrobes;
            }
        }
    }
    else if (choice == "altstrobes" ){
        if (n == 2 ){
            for (auto x: ref_seqs){
                mers_vector altstrobes2; // pos, chr_id, kmer hash value
                altstrobes2 = seq_to_altstrobes2(n, k, k2, w_min, w_max, x.second, x.first);
                tmp_index[x.first] = altstrobes2;
            }
        }
        else {
            for (auto x: ref_seqs){
                mers_vector altstrobes; // pos, chr_id, kmer hash value
                altstrobes = seq_to_altstrobes(n, k, k2, w_min, w_max, x.second, x.first);
                tmp_index[x.first] = altstrobes;
            }
        }
    }
...
//            for (auto & record : records){
                ...
                if (choice == "kmers" ){
                    ...
                    if (n == 2 ){
                        ...
                        query_mers = seq_to_randstrobes(n, k, w_min, w_max, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_randstrobes(n, k, w_min, w_max, seq_rc, q_id);
                    }
                }
                else if (choice == "mixedrandstrobes" ){
                    if (n == 2 ){
                        query_mers = seq_to_mixedrandstrobes2(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_mixedrandstrobes2(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                    }
                    if (n == 3 ){
                        query_mers = seq_to_mixedrandstrobes3(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_mixedrandstrobes3(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                    }
                    else {
                        query_mers = seq_to_mixedrandstrobes(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_mixedrandstrobes(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                    }
                }
                else if (choice == "altstrobes" ){                    
                    if (n == 2 ){
                        query_mers = seq_to_altstrobes2(n, k, k2, w_min, w_max, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_altstrobes2(n, k, k2, w_min, w_max, seq_rc, q_id);
                    }
                    else {
            ...
            //        else if (n == 3){ for (auto x : ref_seqs){generate_minstrobe3_index(h, k, x.second, x.first);}}
                    ...
                    if (n == 2 ){
                        query_mers = seq_to_mixedminstrobes2(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                        seq_rc = reverse_complement(record.seq);
                        query_mers_rc = seq_to_mixedminstrobes2(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                    }
                }
                else if (choice == "hybridstrobes" ){
                    if (n == 2 ){
                       ...
                       for (auto x : ref_seqs){
                            query_mers = seq_to_hybridstrobes(n, k, w_min, w_max, record.seq, q_id);
                            seq_rc = reverse_complement(record.seq);
                            query_mers_rc = seq_to_hybridstrobes(n, k, w_min, w_max, seq_rc, q_id);
                        }
                    }
                    ...
                    if (n == 2 ){
                        for (auto x : ref_seqs){
                            query_mers = seq_to_mixedhybridstrobes2(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                            seq_rc = reverse_complement(record.seq);
                            query_mers_rc = seq_to_mixedhybridstrobes2(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                        }
                    }
                    else if (n == 3){
                        for (auto x : ref_seqs){
                            query_mers = seq_to_mixedhybridstrobes3(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                            seq_rc = reverse_complement(record.seq);
                            query_mers_rc = seq_to_mixedhybridstrobes3(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                        }
                    }
                    else {
                        for (auto x : ref_seqs){
                            query_mers = seq_to_mixedhybridstrobes(n, k, w_min, w_max, strobe_fraction, record.seq, q_id);
                            seq_rc = reverse_complement(record.seq);
                            query_mers_rc = seq_to_mixedhybridstrobes(n, k, w_min, w_max, strobe_fraction, seq_rc, q_id);
                        }
                    }
                }
                // Find NAMs
                ...
                // Remove overlaying duplicates
                nams = remove_duplicates(nams);
                nams_rc = remove_duplicates(nams_rc);
...
//            for (auto & record : records){
        ...
        //        if (!record.qual.empty()) std::cout << record.qual << std::endl;
            ...
            //        else if (n == 3){ for (auto x : ref_seqs){generate_minstrobe3_index(h, k, x.second, x.first);}}
                ...
                #pragma omp critical (datawrite)
                {
                    output_nams(nams_cut, output_file, acc, acc_map, false);
                    output_nams(nams_rc_cut, output_file, acc, acc_map, true);
                };
